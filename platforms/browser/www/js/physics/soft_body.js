...
var SoftBody = (function (_super) {
    ...
    function SoftBody(bodyMesh, samplingRate, renderer) {
        ...
        for (var i = 0; i < this._points.length; i++) {
            for (var j = 0; j < this._points.length; j++) {
                if (j != i) {
                    ...
                    if (distance <= max_distance_structure) {
                        this._constraints.push(new BendConstraint(distance, 0.15, this._points[i], this._points[j], this._renderer));
                    }
            ...
            this._velocityConstraints.push(new FrictionConstraint(this._points[i], 0.3));
        }
